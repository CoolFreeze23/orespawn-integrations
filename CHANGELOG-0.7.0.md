...
## Added
- **Head-anchor table** for all 116 OreSpawn species (104 rigs) at
...
- **Config toggle** `[compat] hats_on_rigs` (default on) to fall back to Hats Renewed's own placement.
- **Table test**: every rig in the OreSpawn jar has an anchor, and every bone an anchor names exists in its rig.
